/**
 * passwordsDontMatch.js
 *
 * A custom response.
 *
 * Example usage:
 * ```
 *     return res.passwordsDontMatch();
 *     // -or-
 *     return res.passwordsDontMatch(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
 *     exits: {
 *       somethingHappened: {
 *         responseType: 'passwordsDontMatch'
 *       }
 *     }
 * ```
 *
...
module.exports = function passwordsDontMatch(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.passwordsDontMatch()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.passwordsDontMatch()` called with an Error:', optionalData);
