/**
 * emailDoesNotExist.js
 *
 * A custom response.
 *
 * Example usage:
 * ```
 *     return res.emailDoesNotExist();
 *     // -or-
 *     return res.emailDoesNotExist(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function emailDoesNotExist(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.emailDoesNotExist()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.emailDoesNotExist()` called with an Error:', optionalData);
